...
expenses.get("/search", (c) => {
  ...
  const result = ExpenseService.search(searchOptions);
...
// Get statistics
expenses.get("/stats/summary", (c) => {
  const userId = c.req.query("userId");
  const startDate = c.req.query("startDate");
  const endDate = c.req.query("endDate");
  
  const stats = ExpenseService.getStatistics(
    userId,
...
expenses.get("/stats/period/:period", (c) => {
  ...
  const stats = ExpenseService.getPeriodTotals(period, userId);
  return c.json(stats);
});

// Get daily totals for a specific month and year
expenses.get("/stats/daily-totals", (c) => {
  ...
  const dailyTotals = ExpenseService.getDailyTotals(yearNum, monthNum, userId);
...
expenses.get("/", (c) => {
  ...
  const allExpenses = ExpenseService.getAll(filters);
...
// Get expense history
expenses.get("/:id/history", (c) => {
  const id = c.req.param("id");
  const history = ExpenseService.getHistory(id);
...
expenses.patch("/:id", async (c) => {
  try {
    ...
    const expense = ExpenseService.update(id, updates);
